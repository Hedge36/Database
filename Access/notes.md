...
| **数字** | **说明**                                                     | **小数精度** | **存储空间大小** | **位** |
| -------- | ------------------------------------------------------------ | ------------ | :--------------- | ------ |
| **字节** | 存储 0 到 255 之间的整数。                                   | 无           | 1 个字节         | 8位    |
| 整型     | 存储 $ \pm2^{15} $ 之间的整数                                | 无           | 2 个字节         | 16位   |
| 长整型   | （默认）存储 $ \pm2^{31} $ 之间的整数                        | 无           | 4 个字节         | 32位   |
| 小数型   | 存储 $–10^{38}$ 到 $10^{38}$ 之间的数字 (.adp)<br />存储 $–10^{28}$ 到 $10^{28}$ 之间的数字（.mdb、.accdb） | 28           | 2 个字节         | 16位   |
| 单精度型 | 存储 $–3.402823e^{38}$ 到 $3.402823e^{38}$ 之间的数。        | 7            | 4 个字节         | 32位   |
| 双精度型 | 存储 $–1.797e^{308}$ 到 $1.797e^{308}$ 之间的正数。          | 15           | 8 个字节         | 64位   |
> **注意：字节型而不是小整数，更不是字符。**
